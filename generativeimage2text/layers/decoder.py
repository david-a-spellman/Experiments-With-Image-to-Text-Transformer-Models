...
def create_decoder(decoder_type, norm_type,
    ...
    if decoder_type is None:
        ...
        gpt_decoder = OpenAIGPTModel.from_pretrained("openai-gpt")
        return gpt_decoder
...
class TransformerDecoderTextualHead(TextualHead):
    ...
    def forward(
        ...
        if self.decoder_type == "bert_en":
            ...
            top_left = torch.zeros((num_memory, num_memory), device=device, dtype=dtype)
            top_right = torch.full((num_memory, num_tgt), float('-inf'), device=tgt.device, dtype=dtype,)
            bottom_left = torch.zeros((num_tgt, num_memory), dtype=dtype, device=uni_mask_zero_neg.device,)
            left = torch.cat((top_left, bottom_left), dim=0)
            right = torch.cat((top_right, uni_mask_zero_neg.to(dtype)), dim=0)
        ...
        elif isinstance(trans_out, BaseModelOutput):
            textual_features = trans_out['last_hidden_state']
        else:
...
class CaptioningModel(nn.Module):
    ...
    def forward(self, caption_tokens, image):
        batch = { "caption_tokens": caption_tokens, "image": image }
    ...
    def forward_one_ce(self, batch, visual_features, visual_features_valid, return_info):
        ...
        if self.training:
            ...
            if self.decoder_type == 'bert_en':
                ...
                feat = torch.permute(output_logits, (1, 0, 2))
                max_cap_len = target.size()[1] - 1
                feat = feat[:, :max_cap_len].contiguous()
